...
struct Box
{
    ...
    */
    ...
    bool intersectFromAbove(Box& other) {
        return (fabs(m_pos_x - other.m_pos_x) * 2.0f < (m_width + other.m_width)) &&
            (fabs(m_pos_y - other.m_pos_y) * 2.0f < (m_height + other.m_height) - 1.0f);

    }

    bool intersectFireRange(Box& other)
    {
        return (fabs(m_pos_x - other.m_pos_x) * 2.0f < ((m_width+ 14.0f)+ other.m_width)) &&
            (fabs(m_pos_y - other.m_pos_y) * 2.0f < ((m_height + 14.0f)  + other.m_height));
    }


    bool inMinimapRange(Box& other ,float  canvasWidth , float canvasHeight)
    {
        return (fabs(m_pos_x - other.m_pos_x) * 2.0f < ((m_width + 4*canvasWidth ) + other.m_width)) &&
            (fabs(m_pos_y - other.m_pos_y) * 2.0f < ((m_height + 6.5*canvasHeight) + other.m_height));
