...
class Net : public GameObject {
	...
	concurrency::concurrent_queue<Data> ad_packets; 
...
public:
	...
	Net(bool host);
	int run();
	void addaudiodata(int id, float arr[]);

	void setOnline(bool a);
	bool getOnline();
	void addpMOVEToQueue(int o_id, float angle, float speed, float x, float y, unsigned long framecounter , bool fireflag);
